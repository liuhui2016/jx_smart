...
<script type="text/javascript">
 $(document).ready(function () {
	...
	$("#filter_EQI_pro_id").attr("value",'${param.filter_EQI_pro_id}');
...
<div class="currloca">
  ...
  <div class="sitemap">
   ...
   <div class="titt">
     <h2><img alt="" src="${ctx}/images/web/311.gif" align="absmiddle" />订单管理</h2>
   </div>
   <form id="mainForm" action="ordersss!lists.action?trade_state=${trade_state}&ord_addtime=<s:date name="ord_addtime" format="yyyy-MM-dd HH:mm:ss"/>&ord_modtime=<s:date name="ord_modtime" format="yyyy-MM-dd HH:mm:ss"/>&ord_managerno=${ord_managerno}" method="post">
    <input type="hidden" name="page.pageNo" id="pageNo" value="${page.pageNo}"/>
    <input type="hidden" name="page.orderBy" id="orderBy" value="${page.orderBy}"/>
    <input type="hidden" name="page.order" id="order" value="${page.order}"/>
    <input type="hidden" name="authId" id="authId" value="1038"/>
  ...
  <!-- 查询条件 -->
   <div id="filter" class="filter">
    ...
    <div class="where">
        <div class="and">
           <span>订单号:</span>
           	<input type="text" class="small" name="ord_no" />
        </div>
        
        <div class="and">
           <span>地址:</span>
           	<input type="text" class="small" name="adr_id" />
        </div>
        
        <div class="and">
           <span>产品经理编号:</span>
           	<input type="text" class="small" name="ord_managerno" />
        </div>
        
        <div class="and">
           <span>产品序列号:</span>
           	<input type="text" class="small" name="pro_no" />
        </div>
       ...
       <div class="and">
           <span>开始时间:</span>
           	<input type="text" class="small" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" name="ord_addtime" id= "ord_addtime"/>
        </div>
        
         <div class="and">
           <span>结束时间:</span>
           	<input type="text" class="small" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" name="ord_modtime" />
        </div>
