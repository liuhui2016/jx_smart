...
public class JxStatisticalServiceImpl extends GenericManagerImpl<JxStatistical,Long>implements IJxStatisticalService{
	...
	public List<Map<String, Object>> findModTds(String uid, String time) {
		String sql = "select sta_tds from jx_statistical where u_id = '"+uid+"' and sta_modtime like '%"+time+"%' ORDER BY sta_modtime DESC ";
	...
	public List<Map<String, Object>> findAddTds(String uid, String time) {
		String sql = "select sta_tds from jx_statistical where u_id = '"+uid+"' and sta_addtime like '%"+time+"%' order by sta_modtime desc ";
		return jdbcTemplate.queryForList(sql);
	}
	...
	public Map<String, Object> findStaAddtime(String uid) {
		...
		String sql = "SELECT S.sta_addtime FROM jx_order O,jx_statistical S where O.u_id = S.u_id and O.u_id = '"+uid+"' and O.ord_protypeid = 0 and O.ord_status = 3 ORDER BY S.sta_addtime DESC LIMIT 1";
	...
	public Map<String, Object> findStaModtime(String uid) {
		...
		String sql = "SELECT S.sta_modtime FROM jx_order O,jx_statistical S where O.u_id = S.u_id and O.u_id = '"+uid+"' and O.ord_protypeid = 0 and O.ord_status = 3 ORDER BY S.sta_modtime DESC LIMIT 1";
		return jdbcTemplate.queryForMap(sql);
	}
	...
	@Override
	public List<Map<String, Object>> findOrderNo(String uid, String sta_modtime2) {
		String sql = "select distinct ord_no from jx_statistical where u_id = '"+uid+"' and sta_modtime like '%"+sta_modtime2+"%' ORDER BY sta_modtime DESC";
		return jdbcTemplate.queryForList(sql);
	}
	...
	//根据订单号查询制水量
	@Override
	public int findOrderWater(String order) {
		String sql = "select output_water from jx_statistical where ord_no = '"+order+"' ORDER BY sta_modtime DESC limit 1";
		return jdbcTemplate.queryForInt(sql);
	}
