...
<div class="currloca">
  ...
  <div class="sitemap">
   ...
   <form id="mainForm" action="partner!rtselectresourcer.action?par_level=${par_level}" method="post">
    ...
    <div id="contral" class="contral pointer" onclick="contral(this);"><img src="${ctx}/images/f_close.gif" title="收起查询面板"/></div>
      ...
      <table class="mtab tab_sel" cellpadding="2" cellspacing="1" border="0">
			...
			<c:forEach items="${pageResourcer.result}" var="a" varStatus="c">
				<tr align="center">
					<td><input type="radio" value="${a.PAR_OTHER}" id="id_${a.id}" name="ids"/></td>
					<td>${a.PAR_OTHER}</td>
					<td name="PAR_NAME_${a.PAR_OTHER}">${a.PAR_NAME}</td>
					<td>
