...
<body>
<div class="currloca">
  <p>${auth.fullMenu}${id==null?'&nbsp;»&nbsp;<span>新增模块</span>':'&nbsp;»&nbsp;<span>修改模块</span>' }</p>
  <div class="sitemap">
...
<div class="container">
  <!-- 内容区域 -->
  <div class="itemtitle"><h2>${id==null?'新增模块':'修改模块' }</h2></div>
  ...
  <form id="inputForm" action="menu!save.action" method="post" enctype="multipart/form-data">
	    ...
	    <div id="indiv" style="width:100%;OVERFLOW-X:auto;">
			...
			<table class="tab_cont" width="" cellspacing="0" cellpadding="0" border="0" align="left">
				<tbody>
				    ...
				    <tr>
						<td class="right"><span class="red">*</span>模块名称:</td>
						<td colspan="3"><input type="text" name="menu_name" size="40" id="menu_name" maxlength="255" value="${menu_name}"/></td>
					</tr>
					
					<tr>
						<td class="right">上级模块id:</td>
						<td colspan="3"><input type="text" name="menu_parentid" size="40" id="menu_parentid" maxlength="255" value="${menu_parentid}"/></td>
					</tr>
					
					<tr>
						<td class="right">模块ico:</td>
						<td colspan="3">
							<input type="file" id="iconfile" name="iconfile" value="${menu_icourl}" style="width: 320px" />
							<input type="hidden" id="menu_icourl" name="menu_icourl" value="${menu_icourl}" />
							<c:if test="${menu_icourl != null && menu_icourl != ''}">
								&nbsp;&nbsp;<a href="#" id="icondelete" onclick="clsGG('${menu_icourl}');">查看原图</a>
							</c:if>
						</td>
					</tr>
