...
$.widget("ui.dialog", {
	...
	_create: function() {
		...
		if (self.options.maximized) {	 			
			uiDialogTitlebar.append('<a href="#" id="dialog-maximize" class="ui-dialog-titlebar-max ui-corner-all"><span>Max</span></a>');
			this.uiDialogTitlebarMax = $('#dialog-maximize', uiDialogTitlebar).hover(function(){
				$(this).addClass('ui-dialog-titlebar-max-hover ui-corner-all ui-state-hover');
			}, function(){
				$(this).removeClass('ui-dialog-titlebar-max-hover ui-corner-all ui-state-hover');
		...
		if (self.options.maximized ) {
			uiDialogTitlebar.append('<a href="#" id="dialog-restore" class="ui-dialog-titlebar-rest ui-corner-all"><span>Restore</span></a>');
				this.uiDialogTitlebarMin = $('#dialog-restore', uiDialogTitlebar).hover(function(){
					$(this).addClass('ui-dialog-titlebar-rest-hover ui-corner-all ui-state-hover');
				}, function(){
					$(this).removeClass('ui-dialog-titlebar-rest-hover ui-corner-all ui-state-hover');
				}).mousedown(function(ev){
					ev.stopPropagation();
				}).click(function(){
					self.restore();
	...
	_maximize: function() {	
		...
		marginHDialog = 25; marginWDialog  = 25;
		//if ($.browser.msie && $.browser.version == 8) {marginHDialog = 25; marginWDialog  = 52;}
		marginHDialog = $(window).height()-marginHDialog;
		marginWDialog = $(window).width()-marginWDialog;
		this.uiDialog.css({left:10, top:$(document).scrollTop()+5 , width:marginWDialog+"px" , height:marginHDialog+"px"});
		$('#dialog-restore').show();
		$('#dialog-maximize').hide();
		$('#dialog-restore').css('right','2em');
		this.size();
		this.uiDialog.css('position','absolute');
		var divid = this.element[0].id
		if (navigator.plugins && navigator.mimeTypes && navigator.mimeTypes.length) { 
			var htmlcontent = $('#'+divid).html();
			$('#'+divid).empty().html(htmlcontent);
		}else{
			 var swfNode="";
			 var ieobj = $('#'+divid).find("OBJECT"); 
			 if(ieobj.length>0){
				 var objparam =$('#'+divid).find('PARAM');
				 swfNode = '<object id="'+ ieobj.attr('id') +'" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="'+ ieobj.attr('width') +'" height="'+ ieobj.attr('height') +'">';
					swfNode += '<param name="movie" value="'+ $(objparam[0]).attr('value') +'" />';
					for(var i=1; i<objparam.length;i++) {
					 swfNode += '<param name="'+$(objparam[i]).attr('name') +'" value="'+ $(objparam[i]).attr('value') +'" />';
					}
				swfNode += "</object>";
				$('#'+divid+' div').empty().html(swfNode);
			 }
		}
	...
	restore: function() {
		...
		this.adjustScrollContent();
		var height='auto';
		if(this.options.height!='auto'){height=(this.uiDialog.height() - 72) + 'px';}
		$('.ui-dialog-content').css({width: 'auto', height: height});
		this._position(this.options.position);
	},	
	size: function() {
		var container = this.uiDialogTitlebar.parent(),
			...
			tbMargin = parseInt(content.css('margin-top')=='auto' ? '0px' : content.css('margin-top'),10) + parseInt(content.css('margin-bottom')== 'auto'? '0px' : content.css('margin-bottom'),10),
			lrMargin = parseInt(content.css('margin-left')=='auto' ? '0px' : content.css('margin-left'),10) + parseInt(content.css('margin-right')== 'auto'? '0px' : content.css('margin-right'),10);
	...
	adjustScrollContent: function () {
		...
		$('.ui-dialog-content').css({width: 'auto', height: heightDialog});
		//$('.ui-dialog').css({width: this.uiDialog.width(), height: this.uiDialog.height()});
	},
...
$.extend($.ui.dialog.overlay, {
	...
	height: function() {
		var scrollHeight,
			offsetHeight;
		// handle IE 6
		if ($.browser.msie && $.browser.version < 9) {//javen 0810  && $.browser.version < 7  http://www.cnblogs.com/a_bu/archive/2011/03/04/1971322.html
...
//			alert(scrollHeight+'px');
	...
	resize: function() {
		...
		$overlays.css({
			...
			//width: $.ui.dialog.overlay.width(),    /**lixb**/
			width: '100%',
