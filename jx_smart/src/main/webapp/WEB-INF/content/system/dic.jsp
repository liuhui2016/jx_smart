...
<div class="container">
  ...
  <form id="mainForm" action="dic.action" method="post">
  ...
  <div id="operate" class="operate">
    <div><security:authorize ifAnyGranted="A_base_DIC_add"><img src="../images/b_add.gif" alt="增加" onclick="opr_input('dic!input.action',false,'${authId}');"/></security:authorize></div>
  </div>
    ...
    <tbody>
		  <tr>
			...
			<c:forEach items="${page.result}" var="c">
				<tr>
					<td height="22"><a href="${ctx}/system/dic!detail.action?id=${c.id}&authId=${authId}">${c.code}</a></td>
					<td>${c.name}</td>
					<td>${c.val1}</td>
					<td>${c.val2}</td>
					<td>${c.description}</td>
					<td>
   						<security:authorize ifAnyGranted="A_base_DIC_add">
                                    <a href="dic!input.action?id=${c.id}&authId=${authId}"><img
