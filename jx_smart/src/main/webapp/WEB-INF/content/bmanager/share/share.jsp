...
<div class="currloca">
  ...
  <div class="sitemap">
   ...
   <form id="mainForm" action="share!list.action" method="post">
   ...
   <div id="filter" class="filter">
    ...
    <div class="where">
        <div class="and">
           <span>分享标题:</span>
           <input type="text" class="small" name="filter_LIKES_share_title" value="${param['filter_LIKES_share_title']}" />
        </div>
    </div>
   ...
   <div class="mainC">
	...
	<div id="operate" class="operate">
	  <div><img src="${ctx}/images/b_add.gif" alt="增加" onclick="opr_input('share!input.action',false,'${authId}');"/></div>
	  <div><img src="${ctx}/images/b_edit.gif" alt="修改" onclick="opr_update('share!input.action','mainForm',false,'${authId}');" /></div>
	  <div><img src="${ctx}/images/b_del.gif" alt="删除" onclick="opr_delete('share!delete.action','mainForm');" /></div>
	</div>
      ...
      <table class="mtab" cellpadding="2" cellspacing="1" border="0">
			...
			<c:forEach items="${page.result}" var="a" varStatus="c">
				<tr align="center">
					...
					<td>${a.share_title}</td>
					<td>${a.share_content}</td>
					<td>${a.share_apkurl}</td>
					<td>${a.share_imgurl}</td>
					<td>${a.share_status==0?'失效':'生效'}</td>
					<td>${a.share_addtime}</td>
				</tr>
			</c:forEach>
		</table>
	</div> 
