 ...
 *
 */
@Namespace("/bmanager/operator")
@Results({ @Result(name = "reload", location = "operator.action?authId=${authId}", type = "redirect") })
public class OperatorAction extends CrudActionSupport<JxPartner>{
    ...
    public JxPartner getModel() {
        if (this.id != null) {
            ...
            if(entity.getPAR_LEVEL().equals("1") || entity.getPAR_LEVEL().equals("2")){
                s_province=pararea;
                s_city="地级市";
                s_county="市、县级市";
            }else if(entity.getPAR_LEVEL().equals("2") || entity.getPAR_LEVEL().equals("-2") ){
    ...
    public String lists() throws Exception {
		...
		int sh = partnerService.updatePartner(ord_managerno);
		page = partnerService.queryProvinces(page,userid);
		return SUCCESS;
	}
    ...
    public String list() throws Exception {
        ...
        System.out.println(entity.getTotal_number());
       /* List<Map<String,Object>> list = partnerService.selectAll();
        for(int i =0;i<list.size();i++){
        	Map<String,Object> map = list.get(i);
        	Long username = (Long) map.get("id");
        	String s = username +"";
        	int numberOfInstalledIsers = jxOrderService.selectNumberOfInstalledIsers(s);
			int numberOfInstalledIsersThreeYears = jxOrderService.selectNumberOfInstalledIsersThreeYears(s);
