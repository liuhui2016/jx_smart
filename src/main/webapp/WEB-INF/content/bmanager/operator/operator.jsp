...
$(document).ready(function () {
	...
	//$("#mainForm").val("${param.filter_EQI_status}");
});
...
<div class="currloca">
  ...
  <form id="mainForm" action="operator!list.action" method="post">
  ...
  <!-- 查询条件 -->
   ...
   <div class="mainC">
	...
	<div id="operate" class="operate" style="display: ${user.username=='admin'?'block':'none'}">
	  ...
	  <div><img src="${ctx}/images/b_edit.gif" alt="修改" onclick="opr_update('operator!input.action','mainForm',false,'${authId}');" /></div>
	  <div><img src="${ctx}/images/b_del.gif" alt="删除" onclick="opr_delete('operator!delete.action','mainForm');" /></div>
	</div>
				...
				<tr align="center">
					...
					<td>
						<c:if test="${a.PAR_LEVEL == 1}">
							<a href="operator!select.action?authId=${authId}&id=${a.id}"><b>${a.id}</b></a>
						</c:if>
					...
					<td>${a.PAR_NAME}</td>
					<td>
						<c:if test="${a.PAR_LEVEL == 1}">运营商</c:if>
						<c:if test="${a.PAR_LEVEL == 2}">创客</c:if>
						<%-- <c:if test="${a.PAR_LEVEL == 3}">区</c:if>
						<c:if test="${a.PAR_LEVEL == 4}">产品经理</c:if> --%>
						<c:if test="${a.PAR_LEVEL == -1}">区县代</c:if>
						<c:if test="${a.PAR_LEVEL == -2}">体验店</c:if>
						<c:if test="${a.PAR_LEVEL == -3}">合伙人</c:if>
					</td>
<%-- 					<input type="hidden" value="${a.parParentid}"></input> --%>
					...
					<td><a href="operators!list.action?authId=${authId}&ord_managerno=${a.id}">详情</a></td>
					
					<td>
					<c:if test="${a.PAR_LEVEL == 1}">已审核</c:if>
					<a href="operator!lists.action?authId=${authId}&ord_managerno=${a.id}" onclick="return confirmAct();"><c:if test="${a.PAR_LEVEL == 2}">待审核</c:if></a>
					</td>
