...
/* 
 function checkArea(){
	...
	var flag = false;
	if(level == -1&&(!(province !="省份"))){
		if(!(province !="北京市" ||province !="上海市" || province !="天津市")){
			
		}else{
			if(!(city!="地级市")){
				$("#areaId").text("区现代必须填写到市!");
			}else{
				$("#areaId").text("区现代必须填写到市!");
			}
		}
	}else if(level == -2&&(!(province!="省份"&&city!="地级市"))){
		if(!(province !="北京市" ||province !="上海市" || province !="天津市")){
			
		}else{
		$("#areaId").text("体验店必须填写到市!");
		}
	}else if((level == -3||level == 4)&&(!(province!="省份"&&city!="地级市"))){
		if(!(province !="北京市" ||province !="上海市" || province !="天津市")){
			
		}else{
		$("#areaId").text("合伙人必须填写到市!");
		}
	}else{
		$("#areaId").text(" ");
		flag = true;
 ...
 function checkArea(){
	...
	if(!(province !="北京市" ||province !="上海市" || province !="天津市")){
		...
		if(level == -1&&(!(province !="省份"&&city!="地级市"))){
			$("#areaId").text("区现代必须填写到市!");
...
function pChange(){
	var level = $("#PAR_LEVEL").val();
	var Parentid = $("#parParentid").val();
	var fl = false;
	var s = /^A{1}[0-9]{5}$/;
	var s2 = /^B{1}[0-9]{7}$/;
	 if(level == -2 && (!s.test(Parentid))){
		$("#parentid").text("请确认合伙人编号");
	}else if(level == -3 && (!s2.test(Parentid)) && (!s.test(Parentid))){
		$("#parentid").text("请确认合伙人编号");
	}else{
		fl = true;
		$("#parentid").text("");
...
<div id="gonggao" class="gonggao" style="display: none">
  ...
  <form id="inputForm" action="partner!rtsave.action" method="post" enctype="multipart/form-data" >
	  <!-- 列表区域-->
				...
				<tbody>
					...
					<tr>
						...
						<td colspan="3">
								<select id="PAR_LEVEL" name="PAR_LEVEL">
									...
									<option value='-1' <c:if test='${PAR_LEVEL== "-1"}'>selected='selected'</c:if>>区县代</option>
									<option value="-2" <c:if test='${PAR_LEVEL== "-2"}'>selected='selected'</c:if>>体验店</option>
									<option value="-3" <c:if test='${PAR_LEVEL== "-3"}'>selected='selected'</c:if>>合伙人</option>
								</select>
						</td>
					</tr>
					...
					<tr>
						...
						<td colspan="3">
							...
							<input type="button" onclick="selectDlg('partner!rtselectresourcer.action', '选择上级合伙人');" value="选择上级合伙人"/>
						</td>
					</tr>
					
					<tr>
						<td class="right"><span class="red">*</span>上级合伙人编号:</td>
						<td colspan="3">
							<input type="text" id="parParentid" name="parParentid" value="${parParentid}" onchange="pChange()" style="width: 320px" /><span id="parentid" style="color:red"></span>
						</td>
					</tr>
					<tr>
