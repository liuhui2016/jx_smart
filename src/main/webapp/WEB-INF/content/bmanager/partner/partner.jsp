			...
			<c:forEach items="${page.result}" var="a" varStatus="c">
				<tr align="center">
					...
					<td>
						...
						<c:if test="${a.PAR_LEVEL == -1}">
							<a href="partner!selectRT.action?authId=${authId}&id=${a.id}"><b>${a.PAR_OTHER}</b></a>
						</c:if>
						<c:if test="${a.PAR_LEVEL == -2}">
							<a href="partner!selectRT.action?authId=${authId}&id=${a.id}"><b>${a.PAR_OTHER}</b></a>
						</c:if>
						<c:if test="${a.PAR_LEVEL == -3}">${a.PAR_OTHER}</c:if>
					</td>
					<td>${a.PAR_NAME}</td>
					<td>
						<c:if test="${a.PAR_LEVEL == 1}">运营商</c:if>
						<c:if test="${a.PAR_LEVEL == 2}">创客</c:if>
						<c:if test="${a.PAR_LEVEL == 3}">创客</c:if>
						<c:if test="${a.PAR_LEVEL == 4}">创客</c:if>
...
<%-- 					<input type="hidden" value="${a.parParentid}"></input> --%>
					...
					<td><a href="orders!list.action?authId=${authId}&ord_managerno=${a.id}">详情</a></td>
