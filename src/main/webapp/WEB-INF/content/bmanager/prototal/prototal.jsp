...
function clsGG(srcpath) {
	...
	$.ajax({ //一个Ajax过程
       ...
       url:'${pageContext.request.contextPath}/smvc/prot/prototal.v',
...
function showdiv(data){
	...
	window.document.getElementById("viewimg").src = data;
...
<div class="currloca">
  ...
  <div class="sitemap">
   ...
   <form id="mainForm" action="prototal!list.action" method="post">
   ...
   <div id="filter" class="filter">
    ...
    <div class="where">
        <div class="and">
           <span>产品分类名称:</span>
           <input type="text" class="small" name="filter_LIKES_prot_name" value="${param['filter_LIKES_prot_name']}" />
        </div>
    </div>
   ...
   <div class="mainC">
	...
	<div id="operate" class="operate">
	  <div><img src="${ctx}/images/b_add.gif" alt="增加" onclick="opr_input('prototal!input.action',false,'${authId}');"/></div>
	  <div><img src="${ctx}/images/b_edit.gif" alt="修改" onclick="opr_update('prototal!input.action','mainForm',false,'${authId}');"/></div>
	  <div><img src="${ctx}/images/b_del.gif" alt="删除" onclick="opr_delete('prototal!delete.action','mainForm');" /></div>
	</div>
...
<!-- 				<th>分类产品新增时间</th> -->
			...
			<c:forEach items="${page.result}" var="a" varStatus="c">
				<tr align="center">
					...
					<td>
						<a href="/jx_smart/bmanager/prodetail/prodetail!select.action?prot_type=${a.prot_type}"><b>${a.prot_name}</b></a>
					</td>
					<td>${a.prod_typename}</td>
					<td>
						&nbsp;&nbsp;<a href="#" id="icondelete" onclick="clsGG('${a.prot_picid}');">查看图片</a>
					</td>
					<td>${a.prod_hz}</td>
					<td>${a.prod_w}</td>
					<td>${a.prod_mpa}</td>
					...
					<td>${a.prod_szi}</td>
					<td>
						<c:if test="${a.prot_status == 0}">上架中</c:if>
						<c:if test="${a.prot_status == 1}">下架</c:if>
					</td>
<%-- 					<td>${a.prot_addtime}</td> --%>
				</tr>
			</c:forEach>
		</table>
	</div> 
