...
function search() {
	...
	$('#odr').val(s);//赋值到隐藏域
	if(s == null || s == ""){
		$('#mainForm').prop("action", "batchs!selects.action");//修改的界面，显示未修改的内容
...
<div class="currloca">
  ...
  <div class="sitemap">
   ...
   <form id="mainForm" action="batch!list.action" method="post">
  ...
  <!-- 查询条件 -->
   <div id="filter" class="filter">
    ...
    <div class="where">
        <div class="and">
           <span>订单号:</span>
           	<input type="text" class="small" name="filter_LIKES_ord_no" value="${param['filter_LIKES_ord_no']} " id = "ordera" />
        </div>
...
<!-- 	</div> -->
      <table class="mtab" cellpadding="2" cellspacing="1" border="0">
			<tr align="center">
			<input type="hidden" name="odr" id="odr" />
			...
			<c:forEach items="${page.result}" var="a" varStatus="c">
				<tr align="center">
					...
					<td id = "orders" name = "orders" value ="${a.ord_no}">${a.ord_no}</td>
					<td>${a.adr_id}</td>
					<td>
			...
			<th class="last" colspan="16">
				<c:if test="${a.trade_state == 0}">
					<a href="batch!inserinto.action?ord_no=${a.ord_no}" style="text-decoration:none"><input type="button" value="加入批量操作"/></a>&nbsp;
				</c:if>
				<c:if test="${a.trade_state != 0}">
					<a href="#" style="text-decoration:none"><span style="color: #E5E0E0"></span></a>
				</c:if>
			...
			<tr>
			<th class="last" colspan="16">
				<c:if test="${a.ord_status == 1 or a.ord_status == 3 or a.ord_status == 4 or a.ord_status == 5}">
				<c:if test="${a.trade_state == 0}">
					<a href="batch!inserinto.action?ord_no=${a.ord_no}" style="text-decoration:none"><input type="button" value="加入批量操作"/></a>&nbsp;
				</c:if>
				<c:if test="${a.trade_state != 0}">
				...
				<c:if test="${a.ord_status == 0 or a.ord_status == 2 or a.ord_status == -1}">
					<a href="#" style="text-decoration:none"><span style="color: #E5E0E0"></span></a>
				</c:if>
				<input class="button" type="button" value="返回" onclick="history.back()"/>		
			</th>
